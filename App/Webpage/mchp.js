...
*/
function getXMLValue(xmlData, device, field) {
	try {
		if(xmlData.getElementsByTagName(device)[0].getElementsByTagName(field)[0].firstChild.nodeValue)
			return xmlData.getElementsByTagName(device)[0].getElementsByTagName(field)[0].firstChild.nodeValue;
		else
			return null;
	} catch(err) { return null; }
}
