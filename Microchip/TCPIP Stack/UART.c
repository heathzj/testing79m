...
BYTE ReadStringUART2(BYTE *Dest, BYTE BufferLen)
{
	BYTE c;
	BYTE count = 0;
...
/************************************************UART2****************************************/
	...
	void WriteUSART2(char data)
	{
	  TXREG2 = data;      // Write the data byte to the USART
	}
	/*
	void getsUSART2(char *buffer, unsigned char len)
	{
	  char i;    // Length counter
	  unsigned char data;
	  ...
	  for(i=0;i<len;i++)  // Only retrieve len characters
	  {
	    while(!DataRdyUSART2());// Wait for data to be received
	...
	void putrsUSART2(const rom char *data)
	{
	  do
	  {  // Transmit a byte
	    while(BusyUSART2());
